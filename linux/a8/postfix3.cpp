...
struct ChildInfo {
	char operation;
	int inputIndex1, inputIndex2, outputIndex;
};

ChildInfo childData[100];
int numThreads = 0;

void *childThread(void *ptr) {
	ChildInfo *childDatap = (ChildInfo *) ptr;

	while (1) {
		int x, y, z;
		read(fds[childDatap->inputIndex1][0], &x, sizeof(int));
		read(fds[childDatap->inputIndex2][0], &y, sizeof(int));

		switch (childDatap->operation) {
		case '+':
			z = x + y;
			break;
		case '-':
		...
		write(fds[childDatap->outputIndex][1], &z, sizeof(int));
	}
...
int main() {
	...
	while (istr >> token) {
		...
		if (isalpha(token)) {
			...
			childData[numThreads].inputIndex2 = indexStack.top();
			indexStack.pop();
			childData[numThreads].inputIndex1 = indexStack.top();
			indexStack.pop();
			//if (!fork())
			//	child(token, operandIndex1, operandIndex2, index);
			childData[numThreads].outputIndex = index;
			childData[numThreads].operation = token;
			pthread_t newThread;
			pthread_create(&newThread, NULL, childThread, (void *) &childData[numThreads]);
			numThreads++;
			//push current index 
			indexStack.push(index);
		}
