		...
		while (fgets(line, MAXLEN, fp)) {
			...
			if (fork() == 0) {
				...
				while(!found){
					...
					fprintf(clientfp, "Enter a letter in word %s ", starredWord);
					fflush(clientfp);
					found = true;
				
					//receive the response. It can be like: trick treat foods 
					char letters[2];
					fscanf(serverfp, "%s/n", letters);
					char letter = letters[0];
					int matched = 0;
					int already = 0;
					for(int i=0; i<strlen(words[index])-1; i++){
						if(starredWord[i] == letter)
							already++;
					
						if(words[index][i] == letter){
							matched++;
							starredWord[i] = letter;
						}
						if(starredWord[i] == '*')
							found = false;
					}
				...
				fprintf(clientfp, "Word: %s You missed: %d times \n", words[index], misses);
				fclose(clientfp);
				fclose(serverfp);
