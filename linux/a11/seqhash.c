...
int main(int argc, char** argv) 
{
  ...
  uint64_t nread = 0;
  ssize_t n = 0;
  //as long as there is more data to read, keep reading!
  ...
  uint32_t hash = 0, hashBL = 0;
  char hashString[100];
  //uint32_t hash = jenkins_one_at_a_time_hash(buffer, fileSize);
   hash = jenkins_one_at_a_time_hash(buffer, BSIZE);
  // hash one blk at a time, combine them
  for(int i=1; i<nblocks; i++){
    hashBL = jenkins_one_at_a_time_hash(&buffer[BSIZE*i], BSIZE);
    // use hash again to combine hash and hashbL
    sprintf(hashString, "%u%u", hash, hashBL);
